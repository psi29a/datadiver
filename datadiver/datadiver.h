...
/**
 * Creates a handle that we use through the life of the file or buffer we are
 * working with.
 *
 * @return DataDiver a handle to our DataDiver context.
 */
DD_SYMBOL DataDiver DataDiver_CreateContext(void);

/**
 * Destroys the DataDiver handle, releases the allocated memory and sets it to null.
 *
 * @param handle a DataDiver context.
...
DD_SYMBOL int DataDiver_OpenBuffer(DataDiver handle, uint8_t *buffer,
		uint32_t size);

DD_SYMBOL int DataDiver_SetOption(DataDiver handle, uint16_t option,
		uint16_t value);

DD_SYMBOL int DataDiver_GetOption(DataDiver handle, uint16_t option);
